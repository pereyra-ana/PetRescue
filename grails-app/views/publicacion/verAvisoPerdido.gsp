	...
	<body>
		...
		<table>
			...
			<div align="left">
				...
				Raza: ${aviso.mascota.raza} <br>
				Sexo: ${aviso.mascota.sexo}</div></td>

			<td><div align="left">
				Nombre: ${aviso.mascota.nombre}<br>
				Tamaño: ${aviso.mascota.tamanio}<br>
				<g:if test="${aviso.mascota.edad}">
					Edad: ${aviso.mascota.edad}</div></td>
				</g:if>
		
			<td><div align="left">
				Provincia: ${aviso.ubicacion.provincia}<br>
				Barrio: ${aviso.ubicacion.barrio}<br>
				Fecha: ${aviso.fecha}</div></td></tr>
		</table>
